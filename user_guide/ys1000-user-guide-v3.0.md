...
| ------------------ | ----------------------------------- | ------------- | ------------ |
| k8s社区版1.17-1.23 | S3兼容的对象存储（minio，qingstor） | NFS           | v1beta1/v1      |
...
从YS1000 v3.0.0版本开始，持有容灾版许可证的用户，可以使用容灾功能（第一个版本仅支持nfs server-client 的数据同步方式）。
...
容灾功能应用的前提条件，需要连接容灾主集群，以及容灾从集群，并且两个集群的数据卷同步需要通过server-client的配置（目前版本仅支持nfs）。
...
-   第二步，下载安装helmtool，并拷贝到可执行目录：
    ...
    ```
    docker cp $(docker create --rm registry.cn-shanghai.aliyuncs.com/jibutech/restore-job:release-3.0.0-latest):/usr/bin/helmtool /tmp/helmtool
    mv /tmp/helmtool /usr/local/bin/helmtool
    ```

-   第三步，在需要做恢复的集群上下载 

    https://github.com/jibutech/docs/blob/main/self-restore/self-restore-3.0.0.sh
