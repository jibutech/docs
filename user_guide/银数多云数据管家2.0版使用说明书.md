...
- [3. 软件配置与授权](#3-软件配置与授权)
- [4. 银数多云数据管家的整体自备份与恢复](#4-银数多云数据管家的整体自备份与恢复)
- [5. 配置集群与备份仓库](#5-配置集群与备份仓库)
    - [5.1 配置待保护Kubernetes集群](#51-配置待保护Kubernetes集群)
    - [5.2 配置备份仓库](#52-配置备份仓库)
    - [53 配置StorageClass转换的ConfigMap](#53-配置StorageClass转换的ConfigMap)
    - [5.4 配置快照](#54-配置快照)
- [6. 备份设置](#6-备份设置)
    - [6.1 创建备份策略](#61-创建备份策略)
    - [6.2 执行备份任务](#62-执行备份任务)
    - [6.3 查看备份作业](#63-查看备份作业)
- [7. 恢复至本集群](#7-恢复至本集群)
...
- [9. 跨集群迁移](#9-跨集群迁移)
    ...
    - [9.2 执行迁移任务](#92-执行迁移任务)
    - [9.3 查看迁移作业](#93-查看迁移作业)
    - [9.4 修改相应应用信息](#94-修改相应应用信息)
    - [9.5 钩子程序](#95-钩子程序)
- [10. 产品限制](#10-产品限制)
- [11. 故障与诊断](#11-故障与诊断)
    - [11.1 日志收集](#111-日志收集)
...
YS1000是以一个应用的方式部署在k8s集群中的，虽然 K8S 集群本身具有平台的冗余性，但意外和灾难总是时有发生，难以完全避免。例如YS1000或者相关的资源由于误操作被误删除，或者YS1000所在的集群被意外破坏等。

YS1000 2.0版实现了自备份与恢复，可以在第一次部署时进行定制化的备份，备份的内容会自动同步到用户配置的S3对象存储中。YS1000的部署也可以进行一个自恢复的过程，从过去备份的时间点中，自动选择最新的恢复点进行恢复。具体的备份与恢复说明，请参考[YS1000 Application Backup & Restore Guide.](https://github.com/jibudata/YS1000/blob/main/design/Implemented/YS1000%20Application%20Backup%20%26%20Restore%20Guide.md)

## 5. 配置集群与备份仓库

### 5.1 配置待保护Kubernetes集群
...
### 5.3 配置StorageClass转换的ConfigMap
