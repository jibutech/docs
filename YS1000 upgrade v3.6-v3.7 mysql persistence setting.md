1. 升级前先在 ys1000 中应用如下的 configmap

```
apiVersion: v1
kind: ConfigMap
metadata:
  name: mysql-env-configmap
data:
  mysql-env.sh: |
    #!/bin/bash
    ...
    export DB_DATA_DIR="${DB_VOLUME_DIR}"
...
```
保存该configmap.yaml 文件，并执行命令：
```
   kubectl -n ys1000 apply -f configmap.yaml
```

2.在helm upgrade ys1000时，在values里面指定mysql的相关参数：
```
mysql:
  image:
    registry: docker.io
  ...
  primary:
    ...
    extraVolumeMounts:
    - name: mysql-scripts
      mountPath: /opt/bitnami/scripts/mysql-env.sh
      subPath: mysql-env.sh
    persistence:
      enabled: true
      existingClaim: mysql
    args:
